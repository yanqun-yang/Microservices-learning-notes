- [微服务学习笔记](#微服务学习笔记)
  ...
  - [OpenFeign](#openfeign)
    ...
    - [日志](#日志)
    - [Q\&A](#qa-2)
      - [如何利用OpenFeign实现远程调用？](#如何利用openfeign实现远程调用)
...
<div align="center">
    <img src="截图/搭建分片集群.png" alt="搭建分片集群" />
</div>
...
&emsp;redis在计算key的hash值不一定是根据整个key计算，分两种情况：
...
&emsp;可以发现，当key的TTL到期以后，再次访问name返回的是nil，说明这个key已经不存在了，对应的内存也得到释放。从而起到内存回收的目的。
...
&emsp;先更新数据的情况，如果此时来了两个线程，一个读一个更新，首先线程1读Redis未命中，读取数据库的旧数据，此时线程2更新了数据库，并且删除了Redis的记录（此时Redis为空，删了也是白删），线程1缓过来了，把旧数据写入Redis，此时Redis也会跟数据库出现TTL时间的不一致。（但是这种情况发生概率极低，要求线程1将旧数据写入Redis的操作要在更新数据库跟删除Redis操作之后，这个条件比较苛刻，因为数据缓存到Redis的速度远快于更新数据库以及删除Redis这两个操作）
...
- 首先需要一个很长很长的二进制数位，默认每一位都是0（Redis提供的好像是2的32次方长度呢）
- 然后需要N个不同算法的哈希函数
...
&emsp;**缓存雪崩**是指在同一时段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力。

解决方案：

- 给不同的Key的TTL添加随机值（30分钟+random(1, 10)分钟）
- 利用Redis**集群**提高服务的可用性
- 给缓存业务添加降级限流策略
- 给业务添加多级缓存（在用户浏览器缓存静态图片；在nginx缓存一些数据，不同在nginx同步数据比较麻烦，所以一般在nginx都是最终一致，也是缓存一些不经常变的数据；在微服务的本地内存缓存一些数据；微服务本地都没有了再去查Redis）
...
&emsp;**缓存击穿问题**也叫**热点Key问题**，就是一个被**高并发访问**并且**缓存重建业务较复杂**的key突然失效了，无数的请求访问会在瞬间给数据库带来巨大的冲击。
...
| ![互斥锁与逻辑过期](截图/互斥锁与逻辑过期.png) |
| :--------------------------------------------: |

&emsp;逻辑过期，即给对应的数据元素添加过期字段，而不在Redis中设置TTL，因此理论上该数据会一直存在于Redis中，因为没有过期时间，所以是逻辑过期（一般用于热点数据，程序员提前把数据缓存到Redis中，这样即使是旧数据，数据库也能应对高并发下的压力，也不会有多数线程来进行数据库的重建缓存数据了）

| ![互斥锁与逻辑过期的比较](截图/互斥锁与逻辑过期的比较.png) |
| :--------------------------------------------------------: |
